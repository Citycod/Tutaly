...
export class ShopModuleUpdates1777335881515 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "shop_products" ADD "imageUrls" text array`,
    );
    await queryRunner.query(
      `ALTER TABLE "shop_products" ADD "downloadCount" integer NOT NULL DEFAULT '0'`,
    );
  }

  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "shop_products" DROP COLUMN "downloadCount"`,
    );
    await queryRunner.query(
      `ALTER TABLE "shop_products" DROP COLUMN "imageUrls"`,
    );

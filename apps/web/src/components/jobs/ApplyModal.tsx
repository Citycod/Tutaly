...
const errorObj = err as { response?: { data?: { message?: string } }, message?: string };
  ...
  const inputClass =
    'w-full rounded-xl border border-c200 px-4 py-3 text-sm text-c900 focus:ring-2 focus:ring-green focus:border-transparent outline-none transition bg-white';
  const errorInputClass =
    'w-full rounded-xl border border-red px-4 py-3 text-sm text-c900 focus:ring-2 focus:ring-red focus:border-transparent outline-none transition bg-red/50';
  const labelClass = 'block text-sm font-semibold text-c700 mb-1.5';

  const FieldError = ({ field }: { field: string }) =>
    fieldErrors[field] ? (
      <p className="text-xs text-red mt-1">{fieldErrors[field]}</p>
    ) : null;
  ...
  const renderStep = () => {
    ...
    if (profileLoading) {
      return (
        <div className="p-16 flex flex-col items-center justify-center">
          ...
          <p className="text-sm text-c500">Preparing your application...</p>
        </div>
      );
    }
    ...
    if (submitted) {
      return (
        <div className="p-10 flex flex-col items-center justify-center text-center">
          <div className="bg-green p-4 rounded-full mb-5">
            <CheckCircle2 className="w-10 h-10 text-green" />
          </div>
          <h3 className="text-xl font-bold text-c900 mb-2">Application Submitted!</h3>
          <p className="text-sm text-c500 mb-6 max-w-xs">
          ...
          <div className="flex gap-3 w-full">
            <button
              ...
              className="flex-1 bg-green text-white font-bold py-2.5 px-4 rounded-xl hover:bg-green transition text-sm"
            >
              View Applications
            </button>
            <button
              onClick={onClose}
              className="flex-1 bg-c100 text-c700 font-bold py-2.5 px-4 rounded-xl hover:bg-c200 transition text-sm"
            >
              Continue Browsing
            </button>
          </div>
    ...
    if (submitting) {
      return (
        <div className="p-16 flex flex-col items-center justify-center">
          ...
          <p className="text-sm font-medium text-c700">
            {uploading ? 'Uploading your resume...' : 'Submitting application...'}
          </p>
          <p className="text-xs text-c400 mt-1">This may take a moment</p>
        </div>
      );
    }
      ...
      return (
        <div className="p-4 sm:p-6 space-y-5">
          <div className="flex items-center gap-2 mb-1">
            ...
            <h3 className="text-lg font-bold text-c900">Personal Information</h3>
          </div>
          <p className="text-sm text-c500 -mt-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="sm:col-span-2">
              ...
              <div className="relative">
                <User className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-c400" />
                <input
                  type="text"
                  value={form.fullName}
            ...
            <div>
              ...
              <div className="relative">
                <Mail className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-c400" />
                <input
                  type="email"
                  value={form.email}
            ...
            <div>
              ...
              <div className="relative">
                <Phone className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-c400" />
                <input
                  type="tel"
                  value={form.phone}
            ...
            <div className="sm:col-span-2">
              ...
              <div className="relative">
                <MapPin className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-c400" />
                <input
                  type="text"
                  value={form.location}
      ...
      return (
        <div className="p-4 sm:p-6 space-y-5">
          <div className="flex items-center gap-2 mb-1">
            ...
            <h3 className="text-lg font-bold text-c900">Resume / CV</h3>
          </div>
          <p className="text-sm text-c500 -mt-3">
          ...
          {hasProfileResume && (
            <button
              ...
              className={`w-full text-left p-4 rounded-xl border-2 transition ${
                useProfileResume
                  ? 'border-green bg-green shadow-sm'
                  : 'border-c200 hover:border-green'
              }`}
            >
              <div className="flex items-center gap-3">
                <div
                  className={`p-2 rounded-lg ${useProfileResume ? 'bg-green' : 'bg-c100'}`}
                >
                  <User className={`w-5 h-5 ${useProfileResume ? 'text-green' : 'text-c500'}`} />
                </div>
                <div className="flex-1">
                  <p className="font-semibold text-c900 text-sm">Use profile resume</p>
                  <p className="text-xs text-c500">
                    Resume already on file from your profile
                  </p>
                </div>
                {useProfileResume && <CheckCircle2 className="w-5 h-5 text-green" />}
              </div>
            </button>
          )}
          ...
          {hasProfileResume && (
            <div className="relative">
              <div className="absolute inset-0 flex items-center">
                <div className="w-full border-t border-c200" />
              </div>
              <div className="relative flex justify-center">
                <span className="bg-white px-3 text-xs text-c400 font-medium">
                  OR
                </span>
              </div>
            </div>
          ...
          <div
            ...
            className={`border-2 border-dashed rounded-xl p-8 text-center cursor-pointer transition ${
              uploadedFile
                ...
                : 'border-c200 hover:border-blueL hover:bg-blueL/20'
            }`}
          >
            {uploadedFile ? (
              ...
              <>
                <Upload className="w-10 h-10 text-c300 mx-auto mb-3" />
                <p className="text-sm font-medium text-c700">
                  {hasProfileResume ? 'Upload a different resume' : 'Upload your resume'}
                </p>
                <p className="text-xs text-c400 mt-1">
          ...
          <div className="flex items-start gap-2 text-xs text-c400 pt-1">
      ...
      return (
        <div className="p-4 sm:p-6 space-y-5">
          <div className="flex items-center gap-2 mb-1">
            ...
            <h3 className="text-lg font-bold text-c900">Professional Background</h3>
          </div>
          <p className="text-sm text-c500 -mt-3">
          ...
          <div>
            ...
            />
            <p className="text-xs text-c400 mt-1">Comma-separated</p>
          </div>
        </div>
      );
    }
    ...
    if (currentStep === 4) {
      return (
        <div className="p-4 sm:p-6 space-y-5">
          <div className="flex items-center gap-2 mb-1">
            <Link2 className="w-5 h-5 text-green" />
            <h3 className="text-lg font-bold text-c900">Links & Expectations</h3>
          </div>
          <p className="text-sm text-c500 -mt-3">
          ...
          <div className="space-y-3">
            <p className="text-xs font-bold uppercase tracking-wider text-c400">
              Online Profiles
            </p>
            ...
            <div className="relative">
              <ExternalLink className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-c700" />
              <input
                type="url"
            ...
            <div className="relative">
              <Globe className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-green" />
              <input
                type="url"
          ...
          <div>
            <label className={labelClass}>
              <span className="flex items-center gap-1.5">
                <Sparkles className="w-4 h-4 text-gold" /> Cover Letter
            ...
            />
            <p className="text-xs text-c400 mt-1 text-right">
      ...
      return (
        <div className="p-4 sm:p-6 space-y-4">
          <div className="flex items-center gap-2 mb-1">
            ...
            <h3 className="text-lg font-bold text-c900">Review Application</h3>
          </div>
          <p className="text-sm text-c500 -mt-2">
            Please review your details before submitting.
          </p>

          {error && (
            <div className="p-3 bg-red border border-red rounded-xl text-sm text-red flex items-start gap-2">
          ...
          <div className="space-y-3 max-h-80 overflow-y-auto pr-1">
            {sections.map((section) => (
              <div
                key={section.title}
                className="bg-c100 rounded-xl p-4 border border-c100"
              >
                <h4 className="text-xs font-bold uppercase tracking-wider text-c400 mb-2.5 flex items-center gap-1.5">
                  ...
                  {section.items
                    ...
                    .map((item) => (
                      <div key={item.label} className="flex items-start gap-2">
                        <span className="text-xs text-c500 w-24 shrink-0 pt-0.5">
                          {item.label}
                        </span>
                        <span className="text-sm text-c900 whitespace-pre-line break-words line-clamp-2">
                          {item.value}
                        </span>
                      </div>
                    ))}
                  {section.items.filter((item) => item.value).length === 0 && (
                    <p className="text-xs text-c400 italic">Not provided</p>
                  )}
                </div>
              </div>
            ))}
            ...
            {form.coverLetter && (
              <div className="bg-c100 rounded-xl p-4 border border-c100">
                <h4 className="text-xs font-bold uppercase tracking-wider text-c400 mb-2 flex items-center gap-1.5">
                  <Sparkles className="w-3.5 h-3.5 text-gold" /> Cover Letter
                </h4>
                <p className="text-sm text-c800 whitespace-pre-line line-clamp-4">
                  {form.coverLetter}
                </p>
              </div>
            )}
  ...
  return createPortal(
    ...
    >
      ...
      <div className="h-full sm:h-auto sm:max-h-screen bg-white sm:rounded-2xl shadow-2xl w-full sm:max-w-xl overflow-hidden flex flex-col sm:absolute sm:top-1/2 sm:left-1/2 sm:transform sm:-translate-x-1/2 sm:-translate-y-1/2">
        ...
        <button
          onClick={onClose}
          className="absolute top-3 right-3 sm:top-4 sm:right-4 p-2 rounded-full hover:bg-c100 transition z-10"
        >
          <X className="w-5 h-5 text-c500" />
        </button>

        {/* Header */}
        <div className="px-4 sm:px-6 pt-4 sm:pt-5 pb-3 border-b border-c100 shrink-0">
          <h2 className="text-base sm:text-lg font-bold text-c900 pr-10">Apply for this role</h2>
          <p className="text-xs sm:text-sm text-c500 truncate">{job.title}</p>
        </div>

        {/* Step Indicator */}
        {showStepper && (
          <div className="px-3 sm:px-6 py-2.5 sm:py-3 border-b border-c100 bg-c100/50 shrink-0">
                      ...
                      <div
                        className={`w-6 h-6 sm:w-8 sm:h-8 rounded-full flex items-center justify-center transition-all ${
                          isDone
                            ? 'bg-green text-white'
                            : isActive
                            ? 'bg-green text-green ring-2 ring-green ring-offset-1'
                            : 'bg-c100 text-c400'
                        }`}
                      >
                        {isDone ? (
                    ...
                    {i < STEPS.length - 1 && (
                      <div
                        className={`flex-1 h-0.5 mx-0.5 sm:mx-1 rounded-full transition-colors ${
                          s.id < currentStep ? 'bg-green' : 'bg-c200'
                        }`}
                      />
                    )}
                  </React.Fragment>
        ...
        {showStepper && currentStep < 5 && (
          <div className="px-4 sm:px-6 py-3 border-t border-c100 flex items-center justify-between shrink-0 bg-white safe-bottom">
            <button
              onClick={prevStep}
              disabled={currentStep === 1}
              className="flex items-center gap-1 text-sm font-medium text-c500 hover:text-c700 disabled:opacity-30 disabled:cursor-not-allowed transition"
            >
            ...
            <button
              onClick={nextStep}
              className="flex items-center gap-1.5 text-sm font-bold text-white bg-green hover:bg-green py-2.5 px-4 sm:px-5 rounded-xl transition shadow-sm"
            >

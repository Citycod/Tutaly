  ...
  if (!token && !error) {
    return (
      <div className="text-center py-8 bg-white/10 backdrop-blur-lg px-4 shadow-2xl sm:rounded-2xl sm:px-10 border border-white/10">
        <h3 className="text-xl font-bold text-red mb-2">Missing Token</h3>
        <p className="text-c400 mb-6">Invalid password reset link.</p>
        <Link href="/auth/signin" className="text-green hover:underline">Return to Sign In</Link>
      </div>
    );
  }
    ...
    >
      {isSuccess ? (
        <div className="text-center py-4">
          ...
          <Link
            href="/auth/signin"
            className="w-full inline-flex justify-center py-3 px-4 rounded-xl text-sm font-bold text-white bg-green hover:bg-green transition-all"
          >
            Go to Sign In
          </Link>
        </div>
        ...
        <form className="space-y-6" onSubmit={handleSubmit}>
          ...
          <div>
            <label className="block text-sm font-medium text-c300">
              New Password
            </label>
            <div className="mt-1 relative rounded-md shadow-sm">
              ...
              <input
                ...
                className="block w-full pl-10 pr-3 py-3 border border-white/10 rounded-xl leading-5 bg-white/5 text-c100 placeholder-gray-500 focus:outline-none focus:border-green sm:text-sm"
          ...
          <div>
            <button
              ...
              className="w-full flex justify-center py-3 px-4 border border-transparent rounded-xl shadow-lg text-sm font-bold text-white bg-gradient-to-r from-green to-green hover:from-green focus:outline-none transition-all disabled:opacity-70"
            >
              {isLoading ? (

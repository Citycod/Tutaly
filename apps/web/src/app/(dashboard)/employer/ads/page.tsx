    ...
    <div>
      <div className="page-header">
        <div className="page-header__content">
          <div className="page-header__title">Ad Campaigns</div>
          <p className="text-sm text-c400 mt-1">Promote your job listings to reach top talent faster.</p>
        </div>
      ...
      <div className="dcard p-0 overflow-hidden">
        <div className="p-6 border-b border-c700">
          <h2 className="font-bold text-c100">All Campaigns</h2>
        </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              <thead className="bg-c800 border-b border-c700 text-xs font-semibold uppercase tracking-wider text-c400">
                <tr>
                  <th className="p-4">Campaign / Job</th>
              ...
              <tbody className="divide-y divide-c700">
                {campaigns.map((camp) => (
                  <tr key={camp.id} className="hover:bg-c800 transition-colors">
                    <td className="p-4">
                      <div className="font-bold text-c100">{camp.name}</div>
                      <div className="text-xs mt-1 text-c400">{camp.jobTitle}</div>
                    </td>
                    <td className="p-4">
                    ...
                    <td className="p-4">
                      <div className="text-sm font-medium text-c100">{formatMoney(camp.total_budget, camp.currency)}</div>
                      <div className="text-xs mt-1 text-c400">Spent {formatMoney(camp.total_spent, camp.currency)}</div>
                    </td>
                    <td className="p-4">
                      <div className="text-sm font-medium text-c100">{camp.impression_count} Views</div>
                      <div className="text-xs mt-1 text-c400">{camp.click_count} Clicks</div>
                    </td>
                    <td className="p-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        {camp.status !== 'ended' && (
                          <button className="p-2 rounded-lg hover:bg-c700 transition-colors text-c400" title={camp.status === 'active' ? 'Pause' : 'Resume'}>
                            {camp.status === 'active' ? <Pause className="w-4 h-4" /> : <Play className="w-4 h-4" />}
                          </button>
                        )}
                        <button className="p-2 rounded-lg hover:bg-c700 transition-colors text-c400" title="Edit Campaign">

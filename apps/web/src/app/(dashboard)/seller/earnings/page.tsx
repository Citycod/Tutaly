    ...
    <div className="max-w-6xl mx-auto py-10 px-4 sm:px-6 lg:px-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-1">Track your revenue, escrow balance, and request payouts.</p>
        </div>
        <button 
          onClick={handleWithdrawal}
          disabled={withdrawing || displayData.availableBalance <= 0}
          className="flex items-center gap-2 bg-gray-900 text-white px-6 py-3 rounded-xl font-bold shadow-lg shadow-gray-900/20 hover:bg-black transition-all disabled:opacity-50"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
        {/* Available Balance */}
        <div className="bg-gradient-to-br from-teal-500 to-teal-700 rounded-2xl shadow-sm border border-teal-600 p-6 text-white relative overflow-hidden">
          <div className="absolute top-0 right-0 -mr-4 -mt-4 w-24 h-24 bg-white opacity-10 rounded-full blur-xl"></div>
          <div className="flex items-center justify-between mb-4">
            <p className="font-bold text-teal-100">Available to Withdraw</p>
            <Wallet className="w-6 h-6 text-teal-200" />
          </div>
          <h2 className="text-4xl font-black mb-1">₦{displayData.availableBalance.toLocaleString()}</h2>
          <p className="text-sm text-teal-100">Cleared funds ready for payout</p>
        </div>

        {/* Escrow Balance */}
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
          <div className="flex items-center justify-between mb-4">
            <p className="font-bold text-gray-500">In Escrow (Pending)</p>
            <Clock className="w-6 h-6 text-amber-500" />
          </div>
          <h2 className="text-3xl font-bold text-gray-900 mb-1">₦{displayData.escrowBalance.toLocaleString()}</h2>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
          <div className="flex items-center justify-between mb-4">
            <p className="font-bold text-gray-500">Total Lifetime Earnings</p>
            <TrendingUp className="w-6 h-6 text-blue-500" />
          </div>
          <h2 className="text-3xl font-bold text-gray-900 mb-1">₦{displayData.totalEarnings.toLocaleString()}</h2>
          <p className="text-sm text-gray-500">Since joining Tutaly</p>
        </div>
      </div>
              ...
              <tbody className="divide-y divide-gray-100">
                {displayData.recentTransactions.map(tx => (
                  <tr key={tx.id} className="hover:bg-gray-50">
                    ...
                    <td className="p-4 capitalize text-gray-600">{tx.type.replace('_', ' ')}</td>

...
import {
  ...
  Shield, Star, Loader2, CheckCircle2, AlertCircle, X, CheckCircle
} from 'lucide-react';
...
alert(err.response?.data?.message || 'Failed to add to cart');
  ...
  const handleRequestQuote = async (e: React.FormEvent) => {
    ...
    if (!quoteDetails.requirements || !quoteDetails.budget) {
      alert('Please fill in the requirements and proposed budget.');
      return;
    }

    setSubmittingQuote(true);
    try {
      await apiAuth.withToken(token).post('/shop/quotes', {
        productId: product.id,
        requirements: quoteDetails.requirements,
        budget: Number(quoteDetails.budget),
        deadline: quoteDetails.deadline ? new Date(quoteDetails.deadline).toISOString() : undefined
      });

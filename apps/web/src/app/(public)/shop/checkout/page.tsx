...
alert(err.response?.data?.message || 'Checkout failed. Please try again.');
  ...
  return (
    <div className="min-h-screen bg-c100 pt-16 pb-16">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <Link href="/shop/cart" className="inline-flex items-center gap-2 text-sm text-c500 hover:text-green mb-8 font-medium">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          ...
          <div className="lg:col-span-2 space-y-8">
            <form id="checkout-form" onSubmit={handleCheckout} className="bg-white rounded-2xl shadow-sm border border-c200 p-8">
              <h2 className="text-xl font-bold text-c900 mb-6 pb-4 border-b">Billing Details</h2>
              
              <div className="space-y-5">
                <div>
                  <label className="block text-sm font-bold text-c700 mb-1">Full Name</label>
                  <input 
                    type="text" 
                    required
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <div>
                    <label className="block text-sm font-bold text-c700 mb-1">Email Address</label>
                    <input 
                      type="email" 
                      required
                      className="w-full bg-c100 border border-c200 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-green" 
                  ...
                  <div>
                    <label className="block text-sm font-bold text-c700 mb-1">Phone Number</label>
                    <input 
                      type="tel" 
                      required
              ...
              <h2 className="text-xl font-bold text-c900 mt-10 mb-6 pb-4 border-b">Payment Method</h2>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <button
                  ...
                  className={`p-4 rounded-xl border-2 text-left transition-all ${
                    gateway === 'flutterwave'
                      ...
                      : 'border-c200 hover:border-orange-300'
                  }`}
                >
                  <div className="flex items-center justify-between">
                    <span className="text-2xl">🦋</span>
                    <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${gateway === 'flutterwave' ? 'border-orange-500' : 'border-c300'}`}>
                      {gateway === 'flutterwave' && <div className="w-2.5 h-2.5 bg-orange-500 rounded-full" />}
                    </div>
                  </div>
                  <p className={`text-base font-bold mt-2 ${gateway === 'flutterwave' ? 'text-orange-700' : 'text-c900'}`}>Flutterwave</p>
                  <p className="text-xs text-c500 mt-1">Pay via Card, USSD, or Bank Transfer</p>
                </button>
                ...
                <button
                  ...
                  className={`p-4 rounded-xl border-2 text-left transition-all ${
                    gateway === 'paystack'
                      ? 'border-blue bg-blueL ring-2 ring-blue ring-opacity-20'
                      : 'border-c200 hover:border-blueL'
                  }`}
                >
                  <div className="flex items-center justify-between">
                    <span className="text-2xl">💳</span>
                    <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${gateway === 'paystack' ? 'border-blue' : 'border-c300'}`}>
                      {gateway === 'paystack' && <div className="w-2.5 h-2.5 bg-blue rounded-full" />}
                    </div>
                  </div>
                  <p className={`text-base font-bold mt-2 ${gateway === 'paystack' ? 'text-blueH' : 'text-c900'}`}>Paystack</p>
                  <p className="text-xs text-c500 mt-1">Pay securely with any Naira card</p>
                </button>
              </div>

              <div className="mt-8 bg-c100 rounded-xl p-4 flex items-start gap-3">
                <ShieldCheck className="w-6 h-6 text-green shrink-0" />
                <p className="text-sm text-c600">
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-2xl shadow-sm border border-c200 p-6 sticky top-24">
              <h3 className="text-lg font-bold text-c900 mb-4 pb-4 border-b flex items-center justify-between">
                Order Summary
                ...
                {cartItems.map((item) => {
                  ...
                  return (
                    <div key={item.productId} className="flex items-start gap-3">
                      <div className="w-12 h-12 bg-c100 rounded-lg flex items-center justify-center shrink-0">
                        {product.imageUrls?.[0] ? (
                          <img src={product.imageUrls[0]} alt="" className="w-full h-full object-cover rounded-lg" />
                        ) : (
                          <ShoppingBag className="w-5 h-5 text-c400" />
                        )}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-bold text-c900 line-clamp-1">{product.title}</p>
                        <p className="text-xs text-c500">Qty: {item.quantity}</p>
                      </div>
                      <div className="text-right">
                        <p className="text-sm font-bold text-c900">
              ...
              <div className="space-y-3 mb-6 pt-4 border-t">
                ...
                <div className="border-t pt-3 flex justify-between">
                  ...
                  <span className="font-black text-2xl text-green">{formatPrice(calculateTotal(), getCartCurrency())}</span>
                </div>
              </div>
              ...
              <div className="mt-6 flex items-center justify-center gap-2 text-xs text-c500">

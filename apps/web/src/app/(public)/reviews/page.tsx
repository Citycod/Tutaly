    ...
    <div className="min-h-screen bg-c100 pt-20 pb-16">
      ...
      <section className="bg-navy py-20 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto text-center relative z-10">
          ...
          <form action="/reviews/search" className="max-w-2xl mx-auto flex gap-2">
            ...
            <button type="submit" className="bg-green hover:bg-green text-white px-8 py-4 rounded-xl font-semibold shadow-lg transition-colors flex items-center gap-2 shrink-0 text-lg">
              Search
            </button>
          </form>
          
          <div className="mt-8 flex items-center justify-center gap-6 text-sm text-c400 font-medium">
            <span className="flex items-center gap-2"><ShieldCheck className="w-5 h-5 text-green" /> 100% Anonymous</span>
            <span className="flex items-center gap-2"><ThumbsUp className="w-5 h-5 text-green" /> Verified Submissions</span>
          </div>
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="flex justify-between items-end mb-10">
          <div>
            <h2 className="text-3xl font-bold text-c900">Popular Companies</h2>
            <p className="text-c500 mt-2">Companies with the most reviews on Tutaly.</p>
          </div>
          <Link href="/reviews/write" className="text-green hover:text-green font-medium flex items-center gap-1 group">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {topCompanies.map((company, index) => (
            <Link key={index} href={`/reviews/company/${encodeURIComponent(company.name.toLowerCase())}`} className="group block">
              <div className="bg-white rounded-2xl shadow-sm border border-c200 p-6 hover:shadow-xl hover:border-green transition-all duration-300 transform group-hover:-translate-y-1">
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-4">
                    <div className="w-14 h-14 rounded-xl bg-c100 border border-c100 flex items-center justify-center shrink-0 group-hover:scale-105 transition-transform">
                      <Building2 className="w-7 h-7 text-c400 group-hover:text-green transition-colors" />
                    </div>
                    <div>
                      <h3 className="text-xl font-bold text-c900 group-hover:text-green transition-colors">{company.name}</h3>
                      <p className="text-sm text-c500">{company.reviews} reviews</p>
                    </div>
                  </div>
                  <div className="bg-green text-green font-bold px-3 py-1 rounded-lg flex items-center gap-1">
                    {company.rating} <Star className="w-4 h-4 fill-current" />
                  </div>
                </div>
                
                <div className="mt-6 pt-6 border-t border-c100 flex items-center justify-between text-sm text-green font-medium">
                  Read Reviews

  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold text-c900">Notifications</h1>
          <p className="text-c500 text-sm mt-1">
        ...
        {unreadCount > 0 && (
          <button
            ...
            className="flex items-center gap-1.5 text-sm font-medium text-green bg-green hover:bg-green px-4 py-2 rounded-xl transition-colors"
          >
      ...
      {loading ? (
            ...
            <div key={i} className="bg-white rounded-2xl border border-c100 p-4 animate-pulse">
              <div className="flex items-center gap-3">
                ...
                <div className="space-y-2 flex-1">
                  <div className="h-4 bg-c200 rounded w-3/4" />
                  <div className="h-3 bg-c100 rounded w-1/4" />
                </div>
              </div>
            </div>
          ))}
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-c100 p-12 text-center">
          ...
          <h3 className="text-lg font-bold text-c900 mb-2">No notifications yet</h3>
          <p className="text-c500 text-sm">When someone interacts with you, you&apos;ll see it here.</p>
        </div>
      ) : (
          ...
          {notifications.map((notif) => (
            <div
              ...
              className={`rounded-2xl border p-4 transition-all cursor-pointer group ${
                notif.isRead
                  ? 'bg-white border-c100 hover:border-c200'
                  : 'bg-green/50 border-green hover:border-green shadow-sm'
              }`}
              onClick={() => !notif.isRead && handleMarkAsRead(notif.id)}
            >
              <div className="flex items-start gap-3">
                <div className="w-10 h-10 rounded-full bg-white border border-c100 flex items-center justify-center text-lg shrink-0 shadow-sm">
                  {typeIcons[notif.type] || '🔔'}
                </div>
                <div className="flex-1 min-w-0">
                  <p className={`text-sm leading-relaxed ${notif.isRead ? 'text-c600' : 'text-c900 font-medium'}`}>
                    {notif.message}
                  </p>
                  <div className="flex items-center gap-3 mt-1.5">
                    ...
                    {notif.link && (
                      <Link
                        href={notif.link}
                        className="text-xs text-green hover:text-green flex items-center gap-1 font-medium"
